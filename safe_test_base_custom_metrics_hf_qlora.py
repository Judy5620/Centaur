...
"""
Evaluate a base model (optionally with a QLoRA adapter) on selected Psych-101
tasks with custom metrics using pure Hugging Face Transformers + PEFT (no Unsloth).

- If adapter_path is given:
    * Loads a base causal LM in 4-bit (bitsandbytes)
    * Loads a PEFT QLoRA adapter on top
- If adapter_path is None/empty:
    * Loads the base model in 16-bit (bf16 if available, else fp16)
...
def preprocess_logits_for_metrics(logits, labels):
    ...
    with torch.no_grad():
        ...
        labels = torch.cat(
            (labels[0, 1:], -100 * torch.ones(1, dtype=torch.long)),
            dim=0,
        )
        logits = logits[0]  # (T, vocab_size)

        # labels 안에 -100(무시 토큰)이 섞여 있으므로,
        # ignore_index=-100 을 반드시 줘야 GPU에서 인덱스 에러가 안 난다.
        ce = torch.nn.functional.cross_entropy(
            logits,
            labels,
            reduction="none",
            ignore_index=-100,  # ← 이게 없으면 -100 때문에 device-side assert 발생 가능
        )  # (T,)
...
def compute_metrics(pred):
    ...
    if pred.predictions.size == 0:
        return {"custom_loss": float("nan")}
    return {"custom_loss": float(pred.predictions.mean())}


# ====== 2. 모델 로딩 (HF + PEFT, 어댑터 optional) ======
...
def load_model(
    ...
    - If adapter_path is provided:
        * Load base model in 4-bit (bitsandbytes) and attach QLoRA adapter (PEFT)
    - If adapter_path is None/empty:
        * Load base model in 16-bit (bf16 if available, else fp16)

    GPU 1장만 쓰도록 device_map을 {"": 0} 으로 고정.
    """
    ...
    if adapter_path is not None and adapter_path.strip() != "":
        ...
        # base model in 4-bit, GPU 0 하나만 사용
        model = AutoModelForCausalLM.from_pretrained(
            base_model_name,
            quantization_config=bnb_config,
            device_map={"": 0},          # ★ GPU 0 한 장만 쓰도록 강제
            torch_dtype=torch.bfloat16,
            attn_implementation="eager",  # SDPA 대신 eager attention 사용 (device-side assert 완화 기대)
        )
    ...
    # 어댑터가 없는 경우: 16bit base model (bf16 또는 fp16)
    else:
        print("[INFO] No adapter_path given → evaluating pure base model only (16-bit).", flush=True)

        # bf16 지원 여부 체크 (A100 등 compute capability >= 8 에서 bf16)
        dtype = torch.float16
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate base model (and optional QLoRA adapter) with custom metrics on Psych-101 tasks (HF + PEFT)."
    )
    parser.add_argument(
        "--base_model",
        type=str,
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "e.g. /path/to/adapter or marcelbinz/Llama-3.1-Centaur-8B-adapter. "
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Name for this evaluation run (used in CSV filename and column). "
    ...
    if run_name is None:
        if args.adapter_path is not None and args.adapter_path.strip() != "":
            ...
            # 어댑터가 없으면 base model 이름 기반으로 run_name 생성
            safe_base = args.base_model.replace("/", "_")
            run_name = f"{safe_base}_base_custom_metrics"
